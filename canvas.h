...
// 図形表示関数群// draw系 は線画で、fill系 は塗りつぶし
...
/* * キーボードやマウスクリック用の関数。 * waitForKey() はキー入力を待ち、その後、getKey() でキーデータ取得 * waitForPoint() はマウスクリックを待ち、その後、getPointedX/Y で座標取得 * waitForCountdown は、指定したミリ秒だけ待つ。 */
  void waitForKey();
  void waitForPoint();
...
/* * 自動再描画の制御 * 通常は、関数呼び出しごとに再描画を要求するので速度が遅い * disableAutoRepaint() を呼んだ後、図形関数を呼ぶと、再描画しない。 * 再描画したいタイミングで、forceRepaint() を呼び出すと良い。 */
